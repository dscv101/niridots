#!/usr/bin/env bash
# Void Linux automated install (STOCK KERNEL ONLY) with:
...
# - NVIDIA GTX 970 (Maxwell) support via DKMS (optional)
#
...
#   2) Save this script and run as root:  bash void-install-stock.sh
...
#   - If something fails, rerun with DEBUG=1 for bash tracing: DEBUG=1 bash void-install-stock.sh
#
set -Eeuo pipefail
...
: "${MIRROR:=https://mirrors.servercentral.com/voidlinux/current}"        # NA/Chicago mirror
...
chattr +C /mnt/swap
# Ensure no compression is applied (best-effort/quiet for older btrfs-progs)
...
# Also set property on the file (best-effort)
...
xbps-install -Sy -y -R "$MIRROR" -r /mnt \
  base-system linux linux-headers btrfs-progs cryptsetup dracut dkms \
...
if $INSTALL_NVIDIA_DKMS; then
  xbps-install -Sy -y -R "$MIRROR_NONFREE" -r /mnt nvidia-dkms || true
  xbps-install -Sy -y -R "$MIRROR_NONFREE" -r /mnt nvidia-libs-32bit || true
else
  xbps-install -Sy -y -R "$MIRROR_NONFREE" -r /mnt nvidia || true
  xbps-install -Sy -y -R "$MIRROR_NONFREE" -r /mnt nvidia-libs-32bit || true
fi
...
if ! chroot /mnt sh -c 'command -v bootctl >/dev/null 2>&1'; then
  xbps-install -Sy -y -R "$MIRROR" -r /mnt systemd-boot || {
...
# Determine installed kernel and initramfs names
...
### ───────────────────────────── NVIDIA driver build ─────────────────────────
# Build DKMS against the installed stock kernel. Capture logs.
if $INSTALL_NVIDIA_DKMS; then
  echo "==> Building NVIDIA DKMS module"
  chroot /mnt /bin/sh -e -c '
    export IGNORE_CC_MISMATCH=1
    command -v dkms >/dev/null 2>&1 || xbps-install -Syu -y -R '"$MIRROR"' dkms
    ...
    # Stock kernel should provide build dir via linux-headers
    [ -e "/lib/modules/$KVER/build" ] || { echo "/lib/modules/$KVER/build missing"; exit 74; }
...
xbps-install -Sy -y -R "$MIRROR" -r /mnt NetworkManager dbus chrony elogind || true
