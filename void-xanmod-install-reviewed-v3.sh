...
# - Btrfs with sensible subvolumes for snapshot/rollback & persistence
...
#   - Swapfile is created on a NOCOW Btrfs subvolume to ensure kernel compatibility.
#   - TRIM is enabled (async) through Btrfs and crypt with 'discard' persisted via /etc/crypttab.
#
set -euo pipefail
...
: "${LOCALE:=en_US.UTF-8}"
: "${KEYMAP:=us}"
: "${SWAP_SIZE:=64G}"              # Swapfile size
: "${MIRROR:=https://alpha.de.repo.voidlinux.org/current}"  # Main repo
: "${MIRROR_NONFREE:=https://alpha.de.repo.voidlinux.org/current/nonfree}"
...
# Build the latest XanMod kernel from source? (true/false)
: "${BUILD_XANMOD:=true}"
...
for bin in sgdisk cryptsetup mkfs.vfat btrfs xbps-install chroot dracut blkid lsblk efibootmgr chattr dd udevadm; do
  require "$bin"
done
...
rereadpt() {
  ...
  # Settle udev events if available
...
echo "==> Creating GPT with EFI and LUKS partitions"
...
echo "==> Setting up LUKS2 on $LUKS_DEV (allow discards for NVMe TRIM)"
...
### ─────────────────────────────── Btrfs setup ───────────────────────────────
echo "==> Creating Btrfs filesystem on $CRYPTROOT"
...
xbps-install -Sy -R "$MIRROR" -r /mnt \
...
xbps-install -Sy -R "$MIRROR" -r /mnt amd-ucode || true
xbps-install -Sy -R "$MIRROR" -r /mnt linux-firmware-amd-ucode || true


# Optional NVIDIA userspace + DKMS (module gets built after kernel is ready)
if $INSTALL_NVIDIA_DKMS; then
  xbps-install -Sy -R "$MIRROR_NONFREE" -r /mnt nvidia-dkms || true
  xbps-install -Sy -R "$MIRROR_NONFREE" -r /mnt nvidia-libs-32bit || true
else
  xbps-install -Sy -R "$MIRROR_NONFREE" -r /mnt nvidia || true
  xbps-install -Sy -R "$MIRROR_NONFREE" -r /mnt nvidia-libs-32bit || true
fi
...
# If bootctl is not present in the chroot, install that package; otherwise proceed.
if ! chroot /mnt sh -c 'command -v bootctl >/dev/null 2>&1'; then
  xbps-install -Sy -R "$MIRROR" -r /mnt systemd-boot || {
    echo "ERROR: 'systemd-boot' package not found in repos. Aborting."
    exit 1
  }
fi

# Ensure /boot is the ESP path inside the chroot
chroot /mnt bootctl --esp-path=/boot install
...
if $BUILD_XANMOD; then
  echo "==> Building the latest XanMod kernel (this can take a while)"
  chroot /mnt /bin/sh -euo pipefail -c "
    xbps-install -Sy -R $MIRROR git gcc make bc flex bison openssl-devel elfutils-devel \
      zstd kmod cpio perl findutils util-linux ncurses-devel dwarves file patch diffutils \
      linux-headers
    cd /usr/src
    ...
    # Use a sane default config; tweak later if needed
    make olddefconfig
    make -j\$(nproc)
    ...
    # Ensure /lib/modules/\$KVER/build points to sources for DKMS
    ln -sfn /usr/src/linux-xanmod /lib/modules/\$KVER/build
    # Initramfs with dracut (include crypt/btrfs)
    dracut -f /boot/$INITRAMFS_NAME \$KVER
  "
else
  echo "==> Skipping XanMod build; using stock Void kernel"
  # Install stock kernel packages
  xbps-install -Sy -R "$MIRROR" -r /mnt linux linux-headers
  chroot /mnt sh -c '
...
# If AMD microcode cpio exists, include it; otherwise omit safely.
EXTRA_INITRD_LINE=""
if [ -f /mnt/boot/amd-ucode.cpio ]; then
  EXTRA_INITRD_LINE="initrd  \\amd-ucode.cpio"
fi

cat > /mnt/boot/loader/entries/void-xanmod.conf <<EOF
...
timeout 3
console-mode max
default ${ENTRY_FILE:-void-xanmod.conf}
EOF

### ───────────────────────────── NVIDIA driver build ─────────────────────────
if $INSTALL_NVIDIA_DKMS && $BUILD_XANMOD; then
  echo "==> Building NVIDIA DKMS module against the installed kernel"
  chroot /mnt /bin/sh -euo pipefail -c "
    xbps-install -Sy -R $MIRROR dkms || true
    xbps-install -Sy -R $MIRROR_NONFREE nvidia-dkms nvidia-libs-32bit || true
    if command -v dkms >/dev/null 2>&1; then
      # Try to detect kernel release from the installed image
      KVER=\$(strings /boot/$KERNEL_IMAGE_NAME | grep -oE '6\.[0-9]+(\.[0-9]+)?-xanmod[^ ]*' | head -n1 || uname -r)
...
# Ensure NVMe scheduler is 'none' (default on modern kernels), keep file for future
...
### ───────────────────────────── Final touches ────────────────────────────────
echo "==> Enabling basic services (runit)"
# Network (NetworkManager optional; use dhcpcd if preferred)
xbps-install -Sy -R "$MIRROR" -r /mnt NetworkManager || true
ln -sf /etc/sv/NetworkManager /mnt/var/service/ || true
# D-Logind (elogind) for desktop sessions
xbps-install -Sy -R "$MIRROR" -r /mnt elogind || true
ln -sf /etc/sv/elogind /mnt/var/service/ || true
xbps-install -Sy -R "$MIRROR" -r /mnt dbus chrony || true
